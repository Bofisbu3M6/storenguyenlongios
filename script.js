...
const API = '';   // same origin; if hosting separately set to your API URL e.g. 'https://api.example.com'
...
async function buyProduct(id, price, name, fileUrl, fileName) {
  ...
  try {
    ...
    currentUser.balance = res.balance || (Number(currentUser.balance) - Number(price));
    document.getElementById('drawer-balance').textContent = Number(currentUser.balance).toLocaleString('vi-VN') + ' VND';
    const info = document.getElementById('purchase-modal-info');
    info.textContent = name + ' — Đã trừ: ' + Number(price).toLocaleString('vi-VN') + ' VND';
...
async function doRegister() {
  ...
  errEl.style.display = 'none';
  if (!cap) { errEl.textContent = 'Vui lòng xác nhận không phải robot'; errEl.style.display = 'block'; return; }
  if (password.length < 6) { errEl.textContent = 'Mật khẩu phải ít nhất 6 ký tự'; errEl.style.display = 'block'; return; }
